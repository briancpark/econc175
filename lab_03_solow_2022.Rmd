...
paper problems. Please answer them on Gradescope. You can even open up
the Gradescope page using an R-command.

```{r}
browseURL("https://gradescope.com/courses/352627")
```
(Note: if this doesn't work, please navigate manually as usual.)
...
$$
\left(20 \over {4 + 1} \right)^{1 \over {1-0.4} } = k = 10.07937
$$
...
```{r}
y_k <- ((20.0 / (-0.5 + 4)) ^ (1 / (1 - 0.4))) ^ 0.4
y_k
```
3.196254. The new steady state output per worker is higher with a faster population growth.
...
(1-0.2) * y_k
```
2.557003, which is also higher with a faster population growth.
...
a) Describe in words what happens in the short run to output. (E.g.,
   ...
   In the short run, ouput increases by around 30% as well, indicating that the increse in technology has made workers more productive. 
...
b) Describe in words what happens in the long-run. (If the long-run
   ...
   In the long run, output increase even more by a slightly higher new steady state. This indicates that the saving function also shifts upward and ahs lead to this slight increase.
   
# 4. Growth and Inequality (skip in 2021 and later)
...
k <- seq(1, 20, .1)
alpha <- 1/3
y.of.k <- k^alpha + (k / 10) ### <--- MODIFY THIS LINE.
## numerical slope = rises/runs
mp.k.numeric = diff(y.of.k)/diff(k)
plot(k[-1], mp.k.numeric)
## analytic derivative, taking derivatitive of k^alpha with respect to k.
## (don't worry about this if you haven't had calculus)
mp.k.analytic = alpha * k^(alpha - 1) + 0.1 ### <--- MODIFY THIS LINE, TOO!
lines(k, mp.k.analytic, col = "red", lwd = 2)
...
4. What level of population growth would maximize income per capita?
   ...
   A low population growth (negative growth) would maximize income per capita. Specifically, a negative growth rate that is larger than the deprecation rate would make the intersection between saving curve and $(n + d)k$ curve as far as possible. Obviously, a negative growth rate is not a plausible goal for society.

## Robots and depreciation? (skip this year)
...
5.  (skip) What would happen to capital per worker if we invested our savings
    ...
    production as traditional capital.) [Two or three sentences is fine.]

6.  (skip) Assume, as per Piketty (and per our 2nd example of a production
    ...
    increase inequality or reduce it? [Two or three sentences is fine.]
...
7. Allow a one-time wave of immigrants, but only those who bring
   exactly the amount of capital needed to leave the steady state
   capital/labor ratio unchanged. What would happen to per capita
   income in the short and long-run? (Hint: think about the time path
   of $k$) [Answer in a sentence or two.]
   
   There would be no change in both the short and the long run, as immigrants bring in capital with them making capital and labor ratio unchanged.
...
8. Allow a one-time wave of penniless immigrants who bring no capital
   ...
   In the short run, there will be a decrease in per capita, as new immigrants are a crutch to the economy with no captial. But in the long run, it will eventually stabilize back to steady state of the original per capita income. 
